...
*
* Header definition of DTree class.
***************************/
#pragma once
...
private:
    ...
    DNode* copyInOrder(DNode *thisNode, DNode *rhsNode);
    DNode* insert(DNode *currNode, DNode *insertMe);
    bool checkReplacementCandidacy(DNode *vacantNode, DNode *replacement);
    DNode* replaceVacant(DNode *vacantNode, DNode *replacement);
    bool remove(DNode *currNode, int disc, DNode *&removed);
    void clear(DNode *node);
    void treeToArr(DNode *node, DNode **arr, int &index);
    DNode* arrToTree(DNode **arr, int leftIndex, int rightIndex);
};
